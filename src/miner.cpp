...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PrintexMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PrintexMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PrintexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("printex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PrintexMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
